...
obter_matrizes_normalizadas_todos_subclusters <- function(true_labels_hiv, name_adj_matrices) {  
 ...
 all_motifs$quant_cluster <- rowSums(all_motifs[, !(names(all_motifs) %in% c("Row_Column", "total"))] != 0) 
 
 ###################################################
 # seleção de motifs exclusivos para cada subtipo
 # indices_unicos <- which(all_motifs$quant_cluster == 1) # altere a quantidade de clusters  
 ...
 colunas_selecionadas <- names(all_motifs)[!(names(all_motifs) %in% c("Row_Column", "total", "quant_cluster"))]  
